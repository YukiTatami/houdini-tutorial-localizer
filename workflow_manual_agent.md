...
□ [シリーズ名]/01_raw_data/ (チャプター別フォルダ含む)
□ [シリーズ名]/02_learning_guide/chapters/ (チャプター別ガイドフォルダ)
□ [シリーズ名]/03_progress/ (進捗管理フォルダ)
...
2. サブフォルダ作成: 
   ...
   - 02_learning_guide/chapters/ (チャプター別ガイド用)
   - 03_progress/ (進捗管理用)
3. 共通情報ファイルのフォルダ構造・命名規則を参照

#### 1-3: 進捗管理ファイル準備指示
ユーザーに以下を依頼：
- progress_tracker.json の作成（03_progress/フォルダ内）
- 各チャプターの初期ステータス設定
- テキストエディタでJSON形式保存
...
ユーザーに以下を指示：
1. 学習ガイド結果をコピー
2. 02_learning_guide/chapters/ フォルダに chapter_{章番号}_{チャプター名}_学習ガイド.md で保存
3. ファイル作成確認
